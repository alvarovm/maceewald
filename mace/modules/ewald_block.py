...
class EwaldBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        k_rbf_values: Optional[torch.Tensor] = None,
        return_k_params: bool = True,
    ):
        super().__init__()
        ...
        self.ewald_scale_sum: Optional[ScaleFactor] = None
    ...
    def forward(
        ...
        if dot.numel() == 0:
            ...
            print("from ewald block b:", b.shape, "from ewald block k:", k.shape)
            dot = torch.sum(torch.gather(k, 0, b) * x.unsqueeze(-2), dim=-1)
        if sinc_damping.numel() == 0:
            if self.use_pbc == False:
                if self.delta_k is None:
                    raise ValueError("delta_k must be provided if use_pbc if False")
                ...
                sinc_damping = torch.tensor(1, dtype=torch.float32)
        ...
        if self.use_pbc:
            ...
            if self.k_rbf_values is None:
                raise ValueError("k_rbf_values must be provided if use_pbc is False")
        ...
        if self.return_k_params:
            ...
            return h_update, torch.tensor([-1.0]), torch.tensor([-1.0])
...
class HadamardBlock(torch.nn.Module):
    ...
    def forward(self, h, bf, idx_s, idx_t):
        ...
        x2 = scatter_sum(x, idx_t, dim=0, dim_size=nAtoms, reduce="sum")
